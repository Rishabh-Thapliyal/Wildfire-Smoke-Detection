...
def setup_logger():
    ...
    file_handler = logging.FileHandler('./logs/logs_val_gemma3.log', mode='w', encoding='utf-8')
...
def parse_llm_fire_response(llm_output):
    """
    Parses an LLM output string to extract fire-related information.

    Args:
        llm_output: A string containing the LLM's response, formatted as shown in the example.

    Returns:
        A dictionary containing the parsed information.  Returns None if the input
    ...
    # Use regex to extract the information in a more robust way
    match = re.search(r"Result:\s*(Yes|No)\s*"
                      r"Confidence:\s*(\d+)\s*"
                      r"Description:\s*(.*)", llm_output, re.DOTALL)  #re.DOTALL allows . to match newlines

    if match:
        parsed_data['Result'] = match.group(1)
        parsed_data['Confidence'] = int(match.group(2))
...
with open("fire_scores_gemma3.pickle", "wb") as file:  # 'wb' = write in binary mode
